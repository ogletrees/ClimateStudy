...
# Fetch each year into a data frame. Clemson has from 1930
for (i in 1930:2016){
...
# These cols are to be the same as the original script expected.
...
# Also make the col name the same. Had to do some NA manipulation.
names(DAY)[2] <- "Temp"
...
# make the day of year var. The original script does this in a dplyr call. I had issues so I'm doing it here.
...
print(p)
#---------------------------------------------------------------
# Step - this adds the white horizontal line at temps
#---------------------------------------------------------------
p <- p +
  ...
  geom_hline(yintercept = 70, colour = "white", linetype=1, size = .25) +
  geom_hline(yintercept = 80, colour = "white", linetype=1, size = .25) +
  geom_hline(yintercept = 90, colour = "white", linetype=1, size = .25) +
  geom_hline(yintercept = 100, colour = "white", linetype=1, size = .25)

print(p)
#---------------------------------------------------------------
# Step - this might add the vertical lines between months
#---------------------------------------------------------------
p <- p +
...
print(p)
#---------------------------------------------------------------
# Step - this adds the axis labels
#---------------------------------------------------------------
p <- p +
...
print(p)
#---------------------------------------------------------------
# Step - this adds the dots and records high and low
#---------------------------------------------------------------
p <- p +
...
# Step
#---------------------------------------------------------------
# the exact placement of these elements will need some trial and error. In 2016 Clemson had no record cold days.
p <- p +
  ...
  annotate("segment", x = 295, xend = 302, y = 77, yend = 82, colour = "firebrick3") +
  annotate("text", x = 333, y = 82, label = "We had 27 days that were the", size=3, colour="firebrick3") +
  annotate("text", x = 324, y = 78, label = "hottest since 1930", size=3, colour="firebrick3")
